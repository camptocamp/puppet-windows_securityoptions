...
describe Puppet::Type.type(:so_systemaccess) do
  ...
  context 'when setting name parameter' do
    ...
    it 'fails with an invalid name' do
      ...
      }.to raise_error(Puppet::ResourceError, %r{Invalid display name: \'Shutdown: Why do you want to shutdown\'})
    end
  ...
  context 'when setting so_value property' do
    ...
    it 'if type integer should fail when passed a string that cannot be converted to an integer' do
      ...
      }.to raise_error(Puppet::ResourceError, %r{Invalid value: \'0whatisit1\'.  This must be a number})
    end
