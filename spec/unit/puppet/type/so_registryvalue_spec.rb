...
describe Puppet::Type.type(:so_registryvalue) do
  ...
  context 'when setting so_value property' do
    ...
    it 'if type 7 should return string if passed a string' do
        resourcemsz[:regvalue] = '0' 
        expect(resourcemsz[:regvalue]).to eq('0')
    end
    it 'if type 7 should return string if passed an integer' do
        resourcemsz[:regvalue] = 0 
        expect(resourcemsz[:regvalue]).to eq('0')
    end
    it 'if type 7 should return string if passed an empty string' do
        resourcemsz[:regvalue] = '' 
        expect(resourcemsz[:regvalue]).to eq('')
    end
   # it 'if type 7 should return string if passed an array with entries' do
