...
describe Puppet::Type.type(:so_systemaccess).provider(:so_systemaccess) do
  ...
  context 'when listing instances' do
    ...
    it 'finds Accounts: Rename guest account (qstring)' do
      example1 = @instances.find do |p|
        p[:name].casecmp('Accounts: Rename guest account'.downcase).zero?
      end
      expect(example1).to eq(name: 'Accounts: Rename guest account',
                             ensure: :present,
                             sovalue: '"Guest_Disabled"')
    end

    it 'finds Network security: Force logoff when logon hours expire (integer)' do
      example2 = @instances.find do |p|
        p[:name].casecmp('Network security: Force logoff when logon hours expire'.downcase).zero?
      end
      expect(example2).to eq(name: 'Network security: Force logoff when logon hours expire',
                             ensure: :present,
                             sovalue: 0)
    end
  end
  ...
  def stub_write_export(value)
    ...
    expect(Dir).to receive(:mkdir).at_least(:once).with(File.join(vardir, 'saimports'))
    writeFile = StringIO.new
    expect(File).to receive(:open).at_least(:once).with('C:\\ProgramData\\PuppetLabs\\Puppet\\cache\\saimports\\enforcepasswordhistory.txt', 'w').and_yield(writeFile)
...
")
  ...
  context 'when updating value' do
    it 'updates value' do
      ...
      stub_flush('C:\\ProgramData\\PuppetLabs\\Puppet\\cache\\saimports\\enforcepasswordhistory.txt')
      provider.flush
    end
  end
end
